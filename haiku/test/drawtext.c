...
static void hDraw(uiAreaHandler *ah, uiArea *area, uiAreaDrawParams *p)
{
	...
	drawText(ctx,
		...
		&bodyFont, 16, 56, 360, uiDrawTextAlignLeft);
	drawText(ctx, "Centered line", &bodyFont, 16, 150, 360, uiDrawTextAlignCenter);
	drawText(ctx, "Right-aligned line", &bodyFont, 16, 172, 360, uiDrawTextAlignRight);
}
...
int main(void)
{
	...
	w = uiNewWindow("libui-ng on Haiku — uiDrawText", 400, 220, 0);
